...
local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Football" })
...
local findClosestBall = function()
	...
	for _, v in next, workspace:GetChildren() do
		...
		if distance < dist then dist = distance; ball = v end
	end
	return ball
end
...
local SectionPhysics = PhysicsTab:CreateSection("Physics Settings", "Left")

local boundaries = {}
if not isPractice then
	pcall(function()
		for _, v in next, workspace.Models.Boundaries:GetChildren() do
			boundaries[#boundaries + 1] = v
		end
	end)
end

local antiBoundaries = SectionPhysics:AddToggle({
	Text = 'Anti OOB', Default = false,
	Callback = function(v)
		for _, b in next, boundaries do
			if b then b.Parent = not v and workspace.Models.Boundaries or nil end
		end
	end
})

local antiBench = SectionPhysics:AddToggle({ Text = 'Anti Bench', Default = false })
local antiJam = SectionPhysics:AddToggle({ Text = 'Anti Jam', Default = false })
local antiJumpCooldown = SectionPhysics:AddToggle({ Text = 'Anti Jump Cooldown', Default = false })
local antiFreeze = SectionPhysics:AddToggle({ Text = 'Anti Freeze', Default = false })
local antiBlock = SectionPhysics:AddToggle({ Text = 'Anti Block', Default = false })

task.spawn(function()
	while true do
		task.wait()
		if not antiBench.State then continue end
		local playerGui = player:FindFirstChild('PlayerGui'); if not playerGui then continue end
		local benchButton = playerGui:FindFirstChild('Benched', true); if not benchButton then continue end
		if benchButton.BackgroundColor3 == Color3.fromRGB(120, 120, 120) then
			pcall(function() replicatedStorage.Remotes.CharacterSoundEvent:FireServer('Game', 'ToggleBench') end)
		end
	end
end)
...
task.spawn(function()
	while true do
		...
		if antiJumpCooldown.State and hum then
			hum:SetStateEnabled(Enum.HumanoidStateType.Jumping, true)
		end
		if antiFreeze.State and hum and hum.WalkSpeed ~= 20 then hum.WalkSpeed = 20 end
		for _, v in next, players:GetPlayers() do
			if v == player then continue end
			local character = v.Character; if not character then continue end
			local eHum = character:FindFirstChildOfClass('Humanoid'); if not eHum then continue end
			for _, part in next, character:GetChildren() do
				if not part:IsA('BasePart') then continue end
				if part.Name == 'Head' or part.Name == 'Torso' then
					if eHum:GetState() == Enum.HumanoidStateType.Running then
						part.CanCollide = not antiJam.State
					else
...
task.spawn(function()
	while true do
		task.wait()
		if not antiBlock.State then continue end
...
-- ==========================================================
-- Visual tab (No Ball Trail + Hide Stadium only)
-- ==========================================================
local VisualTab = Window:CreateTab("Visual")
local SectionVisual = VisualTab:CreateSection("Visual Settings", "Left")

local noBallTrail = SectionVisual:AddToggle({ Text = 'No Ball Trail', Default = false })

workspace.ChildAdded:Connect(function(v)
	if not noBallTrail.State then return end
...
pcall(function()
	for _, v in next, workspace.Models.Stadium:GetDescendants() do
		if not v:IsA('BasePart') then continue end
		stadiumParts[#stadiumParts + 1] = { Part = v, Transparency = v.Transparency, CanCollide = v.CanCollide }
	end
end)

SectionVisual:AddToggle({
	Text = 'Hide Stadium', Default = false,
	Callback = function(v)
...
local SectionCatch = CatchingTab:CreateSection("Catching Settings", "Left")

local footballMagnets = SectionCatch:AddToggle({ Text = 'Footuball Magents', Default = false })
local showCatchHitbox = SectionCatch:AddToggle({ Text = 'Magnets Radius', Default = false })
local magnetsRadius = SectionCatch:AddSlider({ Text = 'Magnets Catch Distance', Min = 0, Max = 25, Default = 0, Decimals = 1 })
local magnetsDelay = SectionCatch:AddSlider({ Text = 'Magnets Catch Delay', Min = 0, Max = 1, Default = 0, Decimals = 1 })
local magsType = SectionCatch:AddDropdown({ Text = 'Magnets Type', List = {'Custom', 'Blatant', 'League', 'Legit'}, Default = 'Custom' })
...
local selectedRange = function()
	...
	if mode == 'legit' then return range / 2
	elseif mode == 'league' then return range / 1.4
	elseif mode == 'blatant' then return range * 1.5 end
	return range
end
...
task.spawn(function()
	while true do
		...
		part.Parent = (footballMagnets.State and showCatchHitbox.State) and workspace.Terrain or nil
		for _, v in next, character:GetChildren() do
			if not v:IsA('BasePart') then continue end
			if not v.Name:lower():find('catch') then continue end
			local distance = (ball.Position - v.Position).Magnitude
			if footballMagnets.State and distance < range then
...
-- Automatics tab
-- ==========================================================
local AutomaticsTab = Window:CreateTab("Automatics")
local SectionAutom = AutomaticsTab:CreateSection("Automatics Settings", "Left")

local autoRush = SectionAutom:AddToggle({ Text = 'Auto Rush', Default = false })
local walkToLine = SectionAutom:AddToggle({ Text = 'Walk To Line', Default = false })
local autoRushReactionTime = SectionAutom:AddSlider({ Text = 'Reaction Time', Min = 0, Max = 1, Default = 0.15, Decimals = 2 })
local autoCatch = SectionAutom:AddToggle({ Text = 'Auto Catch', Default = false })
local autoCatchRadius = SectionAutom:AddSlider({ Text = 'Auto Catch Distance', Min = 0, Max = 25, Default = 0, Decimals = 1 })
local autoSwat = SectionAutom:AddToggle({ Text = 'Auto Swat', Default = false })
local autoSwatRadius = SectionAutom:AddSlider({ Text = 'Auto Swat Distance', Min = 0, Max = 25, Default = 0, Decimals = 1 })

task.spawn(function()
	while true do
		task.wait()
		if not autoRush.State then continue end
		local character = player.Character; if not character then continue end
		local humanoid = character:FindFirstChildOfClass('Humanoid'); if not humanoid then continue end
		local hrp = character:FindFirstChild('HumanoidRootPart'); if not hrp then continue end
		if walkToLine.State then
			if replicatedStorage:FindFirstChild('Flags')
				...
				if line then humanoid:MoveTo(line.Position) end
			end
		end
		for _, v in next, players:GetPlayers() do
			if v == player then continue end
			...
			local football = enemyChar:FindFirstChild('Football'); if not football then continue end
			local footballPart = football:FindFirstChildWhichIsA('BasePart'); if not footballPart then continue end
			if enemyHumanoid.WalkSpeed <= 0 then continue end
			if enemyHumanoid.PlatformStand then continue end
			local ping = 0
			if stats and stats:FindFirstChild('Network') and stats.Network:FindFirstChild('ServerStatsItem')
...
task.spawn(function()
	while true do
		task.wait()
		if not autoCatch.State and not autoSwat.State then continue end
		local character = player.Character; if not character then continue end
		local humanoidRootPart = character:FindFirstChild('HumanoidRootPart'); if not humanoidRootPart then continue end
		local ball = findClosestBall(); if not ball then continue end
		local distance = (ball.Position - humanoidRootPart.Position).Magnitude
		if autoCatch.State and distance <= autoCatchRadius.Value then
			pcall(function()
				replicatedStorage.Remotes.CharacterSoundEvent:FireServer('PlayerActions', isPractice and 'catch ' or 'catch')
			end)
		end
		if autoSwat.State and distance <= autoSwatRadius.Value then
			pcall(function()
...
-- ==========================================================
-- Player tab — Unlock All Gamepass only
-- ==========================================================
local PlayerTab = Window:CreateTab("Player")
local SectionPlayer = PlayerTab:CreateSection("Player Settings", "Left")
...
SectionPlayer:AddButton({
	...
	Callback = function()
		...
		if not ok or not module then return end
		module.PlayerOwnsPass = function(a, b) return true end
	end
})